...
contract NefturiansArtifact is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply, INefturianArtifact {
  ...
  // function useArtifacts(uint256[] calldata tokenIds, uint256[] calldata quantities) public {
  //   require(tokenIds.length == quantities.length, "NA05");
  //   for (uint256 index = 0; index < tokenIds.length; index++) {
  //     require(consumable[tokenIds[index]], "NA06");
  //     require(balanceOf(msg.sender, tokenIds[index]) >= quantities[index], "NA04");
  //   }
  //   _burnBatch(msg.sender, tokenIds, quantities);
  //   emit UseArtifacts(tokenIds, quantities);
  // }
